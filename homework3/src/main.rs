...
fn create_display(max_width: u32, max_height: u32, default_colour: u8) -> Display {
    ...
    Display {
        ...
        cur_width: 0,
        cur_height: 0,
        max_width: max_width as u64,
        max_height: max_height as u64,
    }
}

fn process_commands(display: &mut Display, input: Vec<u64>) {
    let mut cmd_replace = false;
    let mut cmd_chng_clr = false;
    ...
    if cmd_replace {
        ...
        while idx < end {
            if idx % 2 == 0 {
                if input[idx] > display.max_width {
                    panic!();
                }
            } else if input[idx] > display.max_height {
                panic!();
            }
            idx += 1;
        }
        //Установить курсор
        display.cur_width = input[end - 2];
        display.cur_height = input[end - 1];
    }

    if cmd_chng_clr {
        if input[input.len() - 1] > 3 {
            panic!();
        }
        display.matrix.set_colour(
            display.cur_width,
            display.cur_height,
            input[input.len() - 1] as u8,
        );
    }
}
